...
router.get('/', async (req, res) => {
  try {
    ...
    res.render('index', {
      menus: [],
      error: 'Gagal memuat menu: ' + err.message
    });
...
router.get('/orders', async (req, res) => {
  ...
  try {
    ...
    res.render('orders', {
      orders: [],
      error: 'Gagal memuat pesanan: ' + err.message
    });
...
router.post('/orders/:id/cancel', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      "UPDATE orders SET status='Dibatalkan' WHERE id=?",
      [req.params.id]
    );
